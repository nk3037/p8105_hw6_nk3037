...
```{r}
# Comparing the model to two other models
...
cv_error = function(model_formula, data) {
  ...
  cv_results = cv.lm(data, model, m = 5)$delta[1]  
  coef_df = as.data.frame(tidy(model))
  return(coef_df)
}

# Apply the function to each model
cv_results = map_df(models, cv_error, data = birthweight)
